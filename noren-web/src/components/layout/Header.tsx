    ...
    <header className="sticky top-0 z-50 border-b border-slate-700/50 bg-slate-900/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <nav className="flex flex-wrap items-center gap-2 sm:gap-6 text-sm text-slate-300">
          ...
          {/* Auth Section */}
          <div className="ml-2 flex gap-2 sm:gap-3 border-l border-slate-700 pl-2 sm:pl-6 relative">
            {user ? (
              ...
              <>
                <Link
                  href="/auth"
                  className="rounded-lg border border-slate-600 px-3 sm:px-4 py-2 text-slate-300 transition duration-200 hover:border-blue-500 hover:text-blue-400 hover:bg-blue-500/10"
                >
                  ورود | ثبت نام
                </Link>
            ...
            {user && showMenu && (
              <div className="absolute top-full left-0 mt-2 w-48 rounded-lg border border-slate-700/50 bg-slate-800 shadow-lg backdrop-blur-xl overflow-hidden z-50">
